...
int ScanChain( TChain* chain, bool fast = true, int nEvents = -1, string skimFilePrefix = "test") {
  ...
  for(int i = 0; i<numHistos; ++i){
	...
	if (histonames[i]==fullCuts[j].variable || histonames[i]==fullCuts[k].variable){
	  stop = 1;
	  continue;
	}
	vector<Cut> cutSet;
	...
	histonames.push_back(histonames[i] + "_cut_" + cutSet[0].variable + "_" + cutSet[1].variable);
  ...
  for(unsigned int i = 0; i<histonames.size(); ++i){
	...
	if(histonames[i]=="pfmet_cut_n_loose_minus_good_R"){
	  float xbins[7] = {250, 300, 400, 500, 650, 750, 900};
	  histos[mapname] = new TH1F(mapname.c_str(), "", 6, xbins);
	  continue; 
	}
	/*else if(histonames[i]=="pfmet_cut_n_of_first_bjet_R"){
	  float xbins3[6] = {250, 350, 450, 600, 700, 900};
	 ...
	 if(!is_data() && skimFilePrefix.find(string("Signal")) == string::npos) {//this is all background
      ...
      //if(mass_lsp()<250) continue;
      //cout<<"help "<< mass_lsp()<<endl;
	...
	for (unsigned int j = 0; j<cuts.size(); ++j){
	  int flag = 0;
	  if (histonames[i]== cuts[j][0].variable||histonames[i]== cuts[j][1].variable) continue;
	  ...
	  if (flag == 1) continue;
	  mapname = histonames[i] + "_cut_" + cuts[j][0].variable + "_" + cuts[j][1].variable + "_" + samplename;

...
void MakePlots1_1_bin(){
  ...
  for(int i = 0; i<numHistos; ++i){
	...
	if (histonames[i]==cutnames[j]){
	  stop = 1;
	  continue;
	}
	//declare cut histograms
	histonames.push_back(histonames[i] + "_cut_" + cutnames[j]);
	histox.push_back(histox[i] + " with cuts " + cutnames[j]);
    }
  }
...
for(map<string,TH1F*>::iterator h=    hist.begin(); h!=    hist.end();++h) {
  ...
  for(unsigned int i = 0; i<histonames.size(); ++i) {
    ...
    if(histonames[i]=="pt_ISR_cut_n_loose_minus_good"){
      ...
      yield = hist[bgname] -> IntegralAndError(7, numBins, err);
      sg_yield = hist[signame] -> IntegralAndError(7, numBins, sg_err);
      ...
      yield = hist[bgname] -> IntegralAndError(9, numBins, err);
      sg_yield = hist[signame] -> IntegralAndError(9, numBins, sg_err);
      ...
      yield = hist[bgname] -> IntegralAndError(11, numBins, err);
      sg_yield = hist[signame] -> IntegralAndError(11, numBins, sg_err);
      ...
      yield = hist[bgname] -> IntegralAndError(14, numBins, err);
      sg_yield = hist[signame] -> IntegralAndError(14, numBins, sg_err);
      ...
      yield = hist[bgname] -> IntegralAndError(16, numBins, err);
      sg_yield = hist[signame] -> IntegralAndError(16, numBins, sg_err);
    ...
    else if(histonames[i]=="pt_ISR_cut_n_of_first_bjet"){
      int numBins = hist[bgname] -> GetNbinsX();
      string signame = histonames[i] + "_Signal_T2tt_topcorridor";
      double err;
      double sg_err;
      ...
      yield = hist[bgname] -> IntegralAndError(13, numBins, err);
      sg_yield = hist[signame] -> IntegralAndError(13, numBins, sg_err);
      cout<<"---> "<<bgname<<">600 <---"<<endl;
      cout<<"BG Yield: "<<yield<<endl;
      cout<<"SG Yield: "<<sg_yield<<endl;
      cout<<"BG Error: "<<err<<endl; 
      cout<<"SG Error: "<<sg_err<<endl;
    ...
    if(histonames[i]=="pt_ISR_cut_dphi_lep_ptmiss"){
      int numBins = hist[bgname] -> GetNbinsX();
      string signame = histonames[i] + "_Signal_T2tt_topcorridor";
      double err;
      double sg_err;
    ...
    /*if (histonames[i]=="pfmet"||histonames[i]=="pfmet_cut_n_loose_minus_good"||histonames[i]=="pfmet_cut_n_of_first_bjet"||histonames[i]=="pfmet_cut_dphi_lep_ptmiss"){

...
void MakePlots1_3_bin(){
  ...
  for(int i = 0; i<numHistos; ++i){
	...
	for(unsigned int l = k+1; l<cutnames.size(); ++l){
	  if (histonames[i]==cutnames[j]||histonames[i]==cutnames[k]||histonames[i]==cutnames[l]){
	    stop = 1;
	    continue;
	  }
	  //declare cut histograms
	  histonames.push_back(histonames[i] + "_cut_" + cutnames[j] + "_" + cutnames[k] + "_" + cutnames[l]);
	  histox.push_back(histox[i] + " with cuts " + cutnames[j] + ", " + cutnames[k] + ", " + cutnames[l]);
	}
	if(stop==1) continue;
      }
      if(stop==1) continue;
...
for(map<string,TH1F*>::iterator h=    hist.begin(); h!=    hist.end();++h) {
  ...
  for(unsigned int i = 0; i<histonames.size(); ++i) {
    ...
    /* if(histonames[i]=="pfmet_cut_dphi_ISR_ptmiss_dphi_lep_ptmiss_pt_lep1_p4"){
      int numBins = hist[bgname] -> GetNbinsX();
      string signame = histonames[i] + "_Signal_T2tt_topcorridor";
      double err;
      double sg_err;
      double yield = hist[bgname] -> IntegralAndError(9, numBins, err);
      double sg_yield = hist[signame] -> IntegralAndError(9, numBins, sg_err);
      ...
      yield = hist[bgname] -> IntegralAndError(7, 8, err);
      sg_yield = hist[signame] -> IntegralAndError(7, 8, sg_err);
      ...
      yield = hist[bgname] -> IntegralAndError(5, 6, err);
      sg_yield = hist[signame] -> IntegralAndError(5, 6, sg_err);
      ...
      yield = hist[bgname] -> IntegralAndError(3, 4, err);
      sg_yield = hist[signame] -> IntegralAndError(3, 4, sg_err);
    ...
    if (histonames[i]=="pfmet_cut_dphi_ISR_ptmiss_dphi_lep_ptmiss_pt_lep1_p4"||histonames[i]=="pfmet_cut_n_loose_minus_good_dphi_ISR_ptmiss_R"||histonames[i]=="pfmet_cut_dphi_ISR_ptmiss_n_of_first_bjet_R"||histonames[i]=="pfmet_cut_dphi_ISR_ptmiss_dphi_lep_ptmiss_R"){

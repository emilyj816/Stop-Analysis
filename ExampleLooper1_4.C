...
int ScanChain( TChain* chain, bool fast = true, int nEvents = -1, string skimFilePrefix = "test") {
  ...
  hbins.push_back(30);      hlow.push_back(0);       hup.push_back(1500);
  ...
  for(int i = 0; i<numHistos; ++i){
	...
	for (unsigned int l = k+1; l<fullCuts.size(); ++l){
	  for(unsigned int m = l+1; m<fullCuts.size(); ++m){
	    //if ( histonames[i]==fullCuts[j].variable || histonames[i]==fullCuts[k].variable || histonames[i] == fullCuts[l].variable || histonames[i]==fullCuts[m].variable){//don't want repeated cut combinations, don't want to graph variables that aren't part of the cut
	    //stop = 1;
	    //continue;
	    //}
	    vector<Cut> cutSet;
	    ...
	    cutSet.push_back(fullCuts[m]);
	    
	    //declare cut histograms
	    histonames.push_back(histonames[i] + "_cut_" + cutSet[0].variable + "_" + cutSet[1].variable + "_" + cutSet[2].variable + "_" + cutSet[3].variable);
 ...
 //make cut combinations
  for(unsigned int i = 0; i<histonames.size(); ++i){
	...
	if(histonames[i]=="pfmet_cut_dphi_lep_ptmiss_pt_ISR_pt_lep1_p4_R"){
	  float xbins3[6] = {250, 350, 450, 550, 700, 900};
	...
	for (unsigned int j = 0; j<cuts.size(); ++j){
	  int flag = 0;
	  //if(histonames[i]== cuts[j][0].variable||histonames[i]== cuts[j][1].variable||histonames[i] == cuts[j][2].variable||histonames[i] == cuts[j][3].variable) continue;
	  ...
	  if (flag == 1) continue;
	  mapname = histonames[i] + "_cut_" + cuts[j][0].variable + "_" + cuts[j][1].variable + "_" + cuts[j][2].variable + "_" + cuts[j][3].variable + "_" + samplename;

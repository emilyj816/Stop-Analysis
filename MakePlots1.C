...
void MakePlots1_1(){
  ...
  histox.push_back("Minimum #phi [BJet, Lepton]");
  histox.push_back("Minimum #phi [BJet, p_{T}^{miss}]");
  histox.push_back("Minimum #phi [ISR, any}]");
  histox.push_back("Minimum #phi [ISR, BJet}]");
  histox.push_back("Minimum #phi [ISR, any jet}]");
  histox.push_back("Minimum #phi [ISR, lepton}]");
  histox.push_back("Minimum #phi [ISR, p_{T}^{miss}]");
  histox.push_back("Minimum #phi [lepton, p_{T}^{miss}]");
  //histox.push_back("jet_motherid");
  histox.push_back("M_{jj} w/ Min(|M_{jajb}-M_{w}|)[GeV]");
  histox.push_back("M_{t} [GeV]");
  histox.push_back("Number of BJets");
  ...
  histox.push_back("R");
  histox.push_back("R_{bE}");
  histox.push_back("R_{lE}");
  histox.push_back("r_{pt}");
...
for(map<string,TH1F*>::iterator h=    hist.begin(); h!=    hist.end();++h) {
  ...
  for(unsigned int i = 0; i<histonames.size(); ++i) {
    ...
    if(i<numHistos){
      cout<<"yieldBefore for "<<bgname<<": "<<yield<<endl;
      cout<<"Error: "<<err<<endl;
    }
    else{
      cout<<"yieldAfter for "<<bgname<<": "<<yield<<endl;
      cout<<"Error: "<<err<<endl;    
	...
	if (signames[j] == "Signal_T2tt_topcorridor"){
	  double err;
	  ...
	  if(i<numHistos){
	    cout<<"yieldBefore for "<<signame<<": "<<yield<<endl;
	    cout<<"Error: "<<err<<endl;
	  }
	  else{
	    cout<<"yieldAfter for "<<signame<<": "<<yield<<endl;
	    cout<<"Error: "<<err<<endl;  
	  }
	}
      }

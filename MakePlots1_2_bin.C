...
void MakePlots1_2_bin(){
  ...
  for(int i = 0; i<numHistos; ++i){
	...
	if (histonames[i]==cutnames[j]||histonames[i]==cutnames[k]){
	  stop = 1;
	  continue;
	}
	//declare cut histograms
	histonames.push_back(histonames[i] + "_cut_" + cutnames[j] + "_" + cutnames[k]);
	histox.push_back(histox[i] + " with cuts " + cutnames[j] + ", " + cutnames[k]);
      }
      if(stop==1) continue;
    }
  }
...
for(map<string,TH1F*>::iterator h=    hist.begin(); h!=    hist.end();++h) {
  ...
  for(unsigned int i = 0; i<histonames.size(); ++i) {
    ...
    if(histonames[i]=="pfmet_cut_n_loose_minus_good_R"){
      int numBins = hist[bgname] -> GetNbinsX();
      cout<<"help me "<<numBins<<endl;
      string signame = histonames[i] + "_Signal_T2tt_topcorridor";
      double err;
      double sg_err;
      double yield = hist[bgname] -> IntegralAndError(13, numBins, err);
      double sg_yield = hist[signame] -> IntegralAndError(13, numBins, sg_err);

      cout<<"---> "<<bgname<<">750 <---"<<endl;
      cout<<"BG Yield: "<<yield<<endl;
      cout<<"SG Yield: "<<sg_yield<<endl;
      cout<<"BG Error: "<<err<<endl;
      cout<<"SG Error: "<<sg_err<<endl;
      
      yield = hist[bgname] -> IntegralAndError(11, 12, err);
      sg_yield = hist[signame] -> IntegralAndError(11, 12, sg_err);
      cout<<"---> "<<bgname<<">650 <---"<<endl;
      cout<<"BG Yield: "<<yield<<endl;
      cout<<"SG Yield: "<<sg_yield<<endl;
      cout<<"BG Error: "<<err<<endl;
      cout<<"SG Error: "<<sg_err<<endl;

      yield = hist[bgname] -> IntegralAndError(8, 10, err);
      sg_yield = hist[signame] -> IntegralAndError(8, 10, sg_err);
      cout<<"---> "<<bgname<<">500 <---"<<endl;
      cout<<"BG Yield: "<<yield<<endl;
      cout<<"SG Yield: "<<sg_yield<<endl;
      cout<<"BG Error: "<<err<<endl;
      cout<<"SG Error: "<<sg_err<<endl;

      yield = hist[bgname] -> IntegralAndError(6, 7, err);
      sg_yield = hist[signame] -> IntegralAndError(6, 7, sg_err);
      cout<<"---> "<<bgname<<">400 <---"<<endl;
      cout<<"BG Yield: "<<yield<<endl;
      cout<<"SG Yield: "<<sg_yield<<endl;
      cout<<"BG Error: "<<err<<endl; 
      cout<<"SG Error: "<<sg_err<<endl;

      yield = hist[bgname] -> IntegralAndError(4, 5, err);
      sg_yield = hist[signame] -> IntegralAndError(4, 5, sg_err);
      cout<<"---> "<<bgname<<">300 <---"<<endl;
      cout<<"BG Yield: "<<yield<<endl;
      cout<<"SG Yield: "<<sg_yield<<endl;
      cout<<"BG Error: "<<err<<endl; 
      cout<<"SG Error: "<<sg_err<<endl;

      yield = hist[bgname] -> IntegralAndError(3, 3, err);
      sg_yield = hist[signame] -> IntegralAndError(3, 3, sg_err);
      ...
      yield = hist[bgname] -> IntegralAndError(1, 5, err);
      sg_yield = hist[signame] -> IntegralAndError(1, 5, sg_err);
      cout<<"---> "<<bgname<<">0 <---"<<endl;
      cout<<"BG Yield: "<<yield<<endl;
      cout<<"SG Yield: "<<sg_yield<<endl;
      cout<<"BG Error: "<<err<<endl; 
    ...
    /*if (histonames[i]=="pfmet_cut_n_loose_minus_good_dphi_lep_ptmiss"||histonames[i]=="pfmet_cut_dphi_lep_ptmiss_n_of_first_bjet"||histonames[i]=="pfmet_cut_dphi_lep_ptmiss_pt_lep1_p4"){
	...
	for(unsigned int k = (met_bounds[met_bounds.size()-1]-150)/50; k>1; --k){
	  ...
	  //cout<<"help 750 "<<bgname<<" "<<hist[bgname] -> IntegralAndError(12, numBins, errtest)<<" error "<<errtest<<" signal "<<hist[signame] -> IntegralAndError(12, numBins, errtestsig)<<" signal error "<<errtestsig<<endl;

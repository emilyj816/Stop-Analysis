...
int ScanChain( TChain* chain, bool fast = true, int nEvents = -1, string skimFilePrefix = "test") {
  ...
  dphi_lep_ptmiss.cut2 = 1.5;
  fullCuts.push_back(dphi_lep_ptmiss);

  /*Cut n_loose_bjets;
  n_loose_bjets.variable = "n_loose_bjets";
  n_loose_bjets.cut1 = 0;
  n_loose_bjets.cut2 = 0;
  fullCuts.push_back(n_loose_bjets);*/
  ...
  histonames.push_back("csv_max");
  hbins.push_back(20);      hlow.push_back(0);       hup.push_back(1);

  histonames.push_back("csv_min");
  hbins.push_back(20);      hlow.push_back(0);       hup.push_back(1);

  histonames.push_back("csv_sum");
  hbins.push_back(20);      hlow.push_back(0);       hup.push_back(2);

  histonames.push_back("dphi_bjet_lep");
  hbins.push_back(20);     hlow.push_back(0);       hup.push_back(4);

  histonames.push_back("dphi_bjet_ptmiss");
  hbins.push_back(20);     hlow.push_back(0);       hup.push_back(4);

  histonames.push_back("dphi_ISR_any");
  hbins.push_back(20);     hlow.push_back(0);       hup.push_back(4);

  histonames.push_back("dphi_ISR_bjet");
  hbins.push_back(20);     hlow.push_back(0);       hup.push_back(4);

  histonames.push_back("dphi_ISR_jet");
  hbins.push_back(20);     hlow.push_back(0);       hup.push_back(4);

  histonames.push_back("dphi_ISR_lep");
  hbins.push_back(20);     hlow.push_back(0);       hup.push_back(4);
  ...
  //histonames.push_back("jet_motherid");
  //hbins.push_back(10);     hlow.push_back(0);        hup.push_back(10);
  
  histonames.push_back("M_jj");
  hbins.push_back(20);      hlow.push_back(0);       hup.push_back(200);

  histonames.push_back("mt");
  hbins.push_back(20);      hlow.push_back(0);       hup.push_back(1500);

  histonames.push_back("n_bjets");
  hbins.push_back(20);      hlow.push_back(0);       hup.push_back(200);
  ...
  hbins.push_back(10);     hlow.push_back(150);     hup.push_back(1500);
  ...
  histonames.push_back("R_bE");
  hbins.push_back(30);     hlow.push_back(0);       hup.push_back(1);

  histonames.push_back("R_lE");
  hbins.push_back(30);     hlow.push_back(0);       hup.push_back(1);

  histonames.push_back("r_pt");
  hbins.push_back(30);     hlow.push_back(-1);       hup.push_back(1);
  ...
  for(int i = 0; i<numHistos; ++i){
	...
	if (histonames[i]==fullCuts[j].variable){//don't want repeated cut combinations, don't want to graph variables that aren't part of the cut
	  continue;
	}
	  vector<Cut> cutSet;
	  ...
	  histonames.push_back(histonames[i] + "_cut_" + cutSet[0].variable);
  ...
  for(unsigned int i = 0; i<histonames.size(); ++i){
	...
	histos[mapname] = new TH1F(mapname.c_str(), "", hbins[i], hlow[i], hup[i]);
	 ...
	 //here begins the core of this looper - we have no our baseline events - let's investigate those
      ...
      funcMap["dphi_bjet_lep"] = fill_dphi_bjet_lep();
      funcMap["dphi_bjet_ptmiss"] = fill_dphi_bjet_ptmiss();
      funcMap["dphi_ISR_any"] = fill_dphi_ISR_any();
      funcMap["dphi_ISR_bjet"] = fill_dphi_ISR_ptmiss();
      funcMap["dphi_ISR_jet"] = fill_dphi_ISR_jet();
      funcMap["dphi_ISR_lep"] = fill_dphi_ISR_lep();
      funcMap["mt"] = fill_mt();
      funcMap["n_bjets"] = fill_n_bjets();
      funcMap["R_bE"] = fill_R_bE();    
	...
	else if(histonames[i]=="jet_motherid"){
	  mothertag = 1;
	  for(unsigned int i = 0; i<ak4pfjets_p4().size(); ++i){
	    if(/*!ak4pfjets_passMEDbtag()[i]&&ak4pfjets_CSV()[i]<0.5426&&*/nloosebtags()-ngoodbtags()>1){
	      histos[mapname]->Fill(ngoodbtags(), weight);
	      int id = -1;
	      /*float mindR = 0.2;
	      for (unsigned int j = 0; j<genqs_p4().size(); ++j){
		if (dRbetweenVectors(ak4pfjets_p4()[i], genqs_p4()[j])<mindR && genqs_isHardProcess()[i]){
		  mindR = dRbetweenVectors(ak4pfjets_p4()[i], genqs_p4()[j]);
		  id = j;
		}
	      }
	      if (id>-1) histos[mapname]->Fill(abs(genqs_id()[id]), weight);*/
	    }
	  }
	}
	else if (mothertag!=1) { histos[mapname] -> Fill(funcMap[histonames[i] ], weight);
	}
      }
	...
	for (unsigned int j = 0; j<cuts.size(); ++j){
	  int flag = 0;
	  if (histonames[i]== cuts[j][0].variable) continue;
	  ...
	  if (flag == 1) continue;
	  int mothertag = 0;
	  mapname = histonames[i] + "_cut_" + cuts[j][0].variable + "_" + samplename;
	  if(histonames[i]=="jet_motherid"){
	    mothertag = 1;
	    for(unsigned int j = 0; j<ak4pfjets_passMEDbtag().size(); ++j){
	      if (nloosebtags()-ngoodbtags()>1&&!ak4pfjets_passMEDbtag()[j]){ 
		histos[mapname] -> Fill(abs(genqs_motherid()[j]), weight);
	      }
	    }
	  }
	  if (mothertag==1) continue;

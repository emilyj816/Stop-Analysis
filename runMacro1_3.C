{
  ...
  //'compile' macro
  gROOT->ProcessLine(".L ExampleLooper1_3.C+");
  const unsigned int chainsize = 5;//6;
  TChain *ch[chainsize];
  string dataset[chainsize];

  //location of our ntuples that store all the simulated samples
  string babylocation = "/uscms_data/d3/haweber/StopNTuples/";    //>=1lep, >=2j, MET>150, MT>150, minDeltaPhi(j1,j2; MET)>0.5
  ...
  string babylocation2 = "/uscms_data/d3/haweber/StopNTuples/genMETsignal/";    //>=1lep, >=2j, MET>150, MT>150, minDeltaPhi(j1,j2; MET)>0.5
